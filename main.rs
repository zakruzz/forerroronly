...
use opencv::{
    core::{
        Mat, MatTraitConst, MatTraitConstManual, Point, Rect, Scalar, Size, CV_32F,
    },
    dnn,
    highgui,
    imgproc,
...
const DRAW_BOXES: bool = true; // set ke false kalau mau overlay super-ringan
...
/// Preprocess cepat: OpenCV dnn::blob_from_image (SIMD)
fn blob_from_bgr(mat_bgr_u8: &Mat) -> Result<Vec<f32>> {
    let blob = dnn::blob_from_image(
        mat_bgr_u8,
        1.0 / 255.0,                         // scalefactor
        Size::new(INPUT_W, INPUT_H),        // size
        Scalar::new(0.0, 0.0, 0.0, 0.0),    // mean
        true,                                // swapRB (BGR->RGB)
        false,                               // crop
        CV_32F,                              // ddepth
    )?;
    // blob shape = [1,3,640,640] (NCHW)
    let slice = blob.data_typed::<f32>()?; // &[f32] view ke data
    Ok(slice.to_vec()) // copy ke Vec untuk dikirim ke GPU
}
...
/// Decode YOLO head generik: dukung [1,N,5+C] atau [1,5+C,N] atau flat N*(5+C)
fn decode_boxes(out: &[f32], shape: &[usize], num_classes: usize, conf_thr: f32) -> Vec<Det> {
    ...
    if shape.len() >= 3 {
        ...
        if a == stride {
            ...
            for i in 0..n {
                ...
                let mut best_c = 0usize; let mut best_s = 0f32;
...
async fn main() -> Result<()> {
    ...
    let _ = cap.set(videoio::CAP_PROP_FPS, 30.0); // tidak selalu dihormati driver
    ...
    let output_name = output_name.context("no output tensor")?;
    ...
    loop {
        ...
        // preview resize cepat (render lebih ringan)
        imgproc::resize(&frame, &mut disp, Size::new(960, 540), 0.0, 0.0, imgproc::INTER_LINEAR)?;

        // preprocess super-cepat (SIMD)
        let input_tensor = blob_from_bgr(&frame)?;
        ...
        for d in &dets {
            if let Some(name) = classes.get(d.cls) {
                ...
                if lname == "cars" || lname == "motorcyle" || lname == "truck" {
                    ...
                    if DRAW_BOXES {
                        // gambar bbox pada disp (960x540) -> skala dari 640x640
        ...
        fps_counter += 1;
        let now = Instant::now();
        let dt = now.duration_since(fps_last);
        if dt.as_secs_f32() >= 1.0 {
            fps = fps_counter as f32 / dt.as_secs_f32();
            fps_counter = 0;
            fps_last = now;
        }

        // overlay ringkas
        let overlay = format!("FPS: {:.1}   vehicles: {}", fps, total);
