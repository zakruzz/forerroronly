...
pub fn iou(a:&Detection,b:&Detection)->f32 {
    ...
    let area_a=(a.x2-a.x1).max(0.0)*(a.y2-a_y1).max(0.0);
    let area_b=(b.x2-b.x1).max(0.0)*(b.y2-b_y1).max(0.0);
    inter/(area_a+area_b-inter+1e-6)
}

pub fn nms(mut dets: Vec<Detection>, th: f32) -> Vec<Detection> {
    dets.sort_by_key(|d| OrderedFloat(-d.score));
    let mut keep: Vec<Detection> = Vec::new(); // <-- tipe eksplisit
    'outer: for d in dets {
        for k in &keep {
            if d.class_id == k.class_id && iou(&d, k) > th {
                continue 'outer;
            }
        }
        keep.push(d);
...
/// Terima output [1,N,5+num_cls] atau [1,5+num_cls,N]
